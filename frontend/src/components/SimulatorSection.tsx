      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="mb-10 flex flex-col justify-between gap-5 lg:flex-row lg:items-end">
          <div className="max-w-2xl">
            <div className="mb-3 inline-flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-[#b24d31]"><Calculator className="h-4 w-4" /> Simulateur patrimonial</div>
          ...
          <div className="space-y-5 lg:col-span-7">
            ...
            <div className="rounded-lg border border-stone-200 bg-white p-5 sm:p-6">
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <div className="flex gap-4 text-[11px] font-semibold text-stone-600"><span className="flex items-center gap-1.5"><i className="h-2 w-2 rounded-full bg-[#b24d31]" /> Valeur du bien</span><span className="flex items-center gap-1.5"><i className="h-2 w-2 rounded-full bg-emerald-700" /> Capital engagé</span></div>
              </div>
              <svg className="mt-5 h-56 w-full" viewBox="0 0 104 100" role="img" aria-label="Courbe de projection patrimoniale">
                ...
                <polyline points={propertyLine} fill="none" stroke="#b24d31" strokeWidth="0.9" vectorEffect="non-scaling-stroke" />
                <polyline points={investedLine} fill="none" stroke="#5b3a2e" strokeWidth="0.9" vectorEffect="non-scaling-stroke" />
...
const Slider = ({ label, value, display, min, max, step, onChange, icon: Icon, suffix }: { label: string; value: number; display: string; min: number; max: number; step: number; onChange: (value: number) => void; icon: React.ElementType; suffix: string }) => (
  <div className="rounded-lg border border-stone-200 bg-white p-4 shadow-sm">
    <div className="flex items-start justify-between gap-3"><span className="flex items-center gap-2 text-sm text-stone-600"><Icon className="h-4 w-4 text-[#b24d31]" />{label}</span><strong className="whitespace-nowrap font-serif text-lg text-stone-900">{display}</strong></div>
    <input aria-label={label} className="mt-5 w-full accent-[#b24d31]" type="range" min={min} max={max} step={step} value={value} onChange={(event) => onChange(Number(event.target.value))} />
...
const Metric = ({ label, value, accent = false, positive }: { label: string; value: string; accent?: boolean; positive?: boolean }) => (
  <div className={`rounded-lg border p-5 ${accent ? 'border-[#e9cfc5] bg-[#fff8f5]' : 'border-stone-200 bg-white'}`}>
    ...
    <p className={`mt-2 font-serif text-2xl font-bold ${accent ? 'text-[#b24d31]' : positive === false ? 'text-red-600' : 'text-stone-900'}`}>{value}</p>
  </div>
);