      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center mb-16">
          ...
          <div className="lg:col-span-7 relative rounded-2xl overflow-hidden bg-neutral-900 aspect-16/10 border border-neutral-200 shadow-md group">
            <img
              src="./images/photo actuel du chantier.jpeg"
            ...
            <div className="absolute top-4 left-4 bg-emerald-600 text-white text-xs font-bold px-3 py-1.5 rounded-md shadow-md flex items-center gap-1.5">
              ...
              <span>Photo Réelle de Chantier • 2026</span>
            </div>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
            {timelineMilestones.map((m) => {
              ...
              const isFirstMilestone = m.stage_number === 1;
              const isFirstMilestoneDone = isFirstMilestone && isDone;
                ...
                <div
                  ...
                  className={`p-5 rounded-2xl border transition-all flex flex-col justify-between space-y-4 ${
                    isInProgress
                      ...
                      : isDone && !isFirstMilestone
                      ? 'bg-emerald-50/40 border-emerald-200'
                      : isFirstMilestoneDone
                      ? 'bg-[#b39776] border-[#a98159] text-white shadow-sm'
                      : 'bg-white border-neutral-200/80 opacity-80'
                  }`}
                >
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      <span className={`text-[11px] font-mono font-bold px-2 py-0.5 rounded ${isFirstMilestoneDone ? 'bg-white/20 text-white' : 'bg-neutral-100 text-neutral-800'}`}>
                        Étape 0{m.stage_number}
                      </span>
                      
                      {isDone ? (
                        <span className={`text-[11px] font-bold flex items-center gap-1 ${isFirstMilestoneDone ? 'text-white' : 'text-[#c48f5f]'}`}>
                          <CheckCircle2 className="w-3.5 h-3.5" /> Achevé
                        </span>
                      ) : isInProgress ? (
                        <span className="text-[11px] font-bold text-[#c48f5f] flex items-center gap-1">
                          <span className="w-2 h-2 rounded-full bg-[#c48f5f] animate-ping" /> En cours
                        </span>
                      ) : (
                    ...
                    <h4 className={`text-sm font-bold leading-snug ${isFirstMilestoneDone ? 'text-white' : 'text-neutral-900'}`}>
                      {m.title}
                    </h4>

                    <p className={`text-xs leading-relaxed line-clamp-3 ${isFirstMilestoneDone ? 'text-white/90' : 'text-neutral-500'}`}>
                      {m.description}
                    </p>
                  </div>
                  ...
                  <div className="space-y-2 pt-2 border-t border-neutral-100">
                    <div className="flex justify-between text-[11px]">
                      <span className={`${isFirstMilestoneDone ? 'text-white/90' : 'text-neutral-500'} font-medium`}>{m.date_display}</span>
                      <span className={`font-mono font-bold ${isFirstMilestoneDone ? 'text-white' : 'text-neutral-900'}`}>{m.progress_percent}%</span>
                    </div>

                    <div className="w-full bg-neutral-100 h-1.5 rounded-full overflow-hidden">
                      <div
                        className={`h-full rounded-full ${
                          isDone && !isFirstMilestone ? 'bg-[#c48f5f]' : isInProgress ? 'bg-[#c48f5f]' : isFirstMilestoneDone ? 'bg-white' : 'bg-neutral-300'
                        }`}
                    ...
                    {m.is_3d_render && (
                      <span className={`text-[9px] uppercase font-semibold block text-right ${isFirstMilestoneDone ? 'text-white/90' : 'text-neutral-600'}`}>
                        Perspective 3D
                      </span>
                    )}
                  </div>
