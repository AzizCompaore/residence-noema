...
import { EmergenceProgramSection } from './components/EmergenceProgramSection';
...
export function App() {
  ...
  const [selectedProjectPurpose, setSelectedProjectPurpose] = useState<string | undefined>(undefined);
  ...
  const handleSelectProjectProfile = (purpose: string) => {
      ...
      {!loading && showLogin ? (
        ...
        <>
          ...
          <main className="flex-1">
            {/* Section 1: Hero Section — positionnement diaspora */}
            <HeroSection
              residence={residence}
              onOpenSimulator={() => handleOpenSimulator()}
              onOpenVisioAppointment={() => setIsVisioAppointmentOpen(true)}
            ...
            {/* Galerie des espaces communs : hall, piscine et parking */}
            <RevealSection delay={80}><DiscoverSection /></RevealSection>

            {/* Section 4: Votre adresse à Abidjan — expérience & prestations diaspora */}
            <RevealSection><ExperienceSection /></RevealSection>

            {/* Section 4: Les Appartements Disponibles */}
            ...
            {/* Section 5: NOEMA, Un Projet Réel (Avancement Chantier) */}
            <RevealSection delay={120}><ConstructionSection milestones={milestones} /></RevealSection>

            {/* Section 6: Le mot de Mme Bamba & présentation Uriel Group */}
            <RevealSection><StoryVideoSection /></RevealSection>

            {/* Section 8: Pourquoi Investir dans NOEMA ? */}
            <RevealSection><WhyInvestSection /></RevealSection>

            {/* Section 9: Financement & Accompagnement Diaspora + parcours en 7 étapes */}
            <RevealSection delay={80}><ImmoDiaspoSection onOpenSimulator={() => handleOpenSimulator()} /></RevealSection>

            {/* Section 10: Programme Émergence & réassurance financement bancaire */}
            <RevealSection><EmergenceProgramSection /></RevealSection>

            {/* Section 11: Localisation (Angré, Abidjan) */}
            <RevealSection><LocationSection /></RevealSection>

            {/* Section 12: FAQ Interactive */}
          ...
          <SimulatorModal
            ...
            whatsappNumber={residence?.whatsapp_number}
            initialProjectPurpose={selectedProjectPurpose}
          />
