...
const DELIVERY_REFERENCE_DATE = new Date('2026-09-15T00:00:00Z');
const ESTIMATED_DELIVERY_DATE = new Date(DELIVERY_REFERENCE_DATE);
ESTIMATED_DELIVERY_DATE.setUTCMonth(ESTIMATED_DELIVERY_DATE.getUTCMonth() + REMAINING_DELIVERY_MONTHS);
const DELIVERY_DATE_LABEL = ESTIMATED_DELIVERY_DATE.toLocaleDateString('fr-FR', {
  month: 'long',
  year: 'numeric',
  timeZone: 'UTC'
});
...
let faqItems: FAQItem[] = [
  ...
  },
  {
    ...
    answer: `La durée totale annoncée pour la Résidence NOEMA est de ${TOTAL_DELIVERY_MONTHS} mois. Deux mois étant déjà réalisés, la livraison prévisionnelle est fixée au ${DELIVERY_DATE_LABEL}. Le planning est suivi par la direction des travaux.`,
    display_order: 7,
    is_published: true
  }
];
...
app.post('/api/leads', async (req: Request, res: Response) => {
  try {
    ...
    res.status(502).json({ success: false, error: 'Le service de collecte des prospects est momentanément indisponible.' });
  }
});
...
app.get('/api/leads', async (req: Request, res: Response) => {
  try {
    ...
    res.status(502).json({ success: false, error: 'Le service des prospects est momentanément indisponible.' });
  }
});
...
3. Données réelles vérifiées de NOEMA :
  ...
  - Livraison annoncée : ${residence?.delivery_date_estimated || DELIVERY_DATE_LABEL}.
...
`;
    ...
    if (requestedApartment && (lower.includes('composition') || lower.includes('surface') || lower.includes('balcon') || lower.includes('pièce') || lower.includes('prix') || lower.includes('tarif') || lower.includes('coût'))) {
      ...
      reply = `Le chantier de la Résidence NOEMA est actuellement en cours (élévation du gros œuvre R+7). Deux mois de travaux étant déjà réalisés sur les ${TOTAL_DELIVERY_MONTHS} mois annoncés, la livraison prévisionnelle est fixée au ${DELIVERY_DATE_LABEL}, avec un suivi rigoureux des normes techniques.`;
